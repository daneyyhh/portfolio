...
export default function EngineeringProfile({ engineerMode }) {
  const getIcon = (id) => {
    switch (id) {
      case '01': return <Globe className="text-[#FF1E27]" size={28} />;
      case '02': return <Bot className="text-[#FF1E27]" size={28} />;
      case '03': return <Layout className="text-[#FF1E27]" size={28} />;
      case '04': return <Gamepad2 className="text-[#FF1E27]" size={28} />;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {engineeringDomains.map((domain, index) => (
            <motion.div
              ...
              className="bg-[#141414] border border-white/15 p-8 rounded-none hover:border-[#FF1E27]/50 transition-all duration-300 group flex flex-col justify-between"
            >
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-mono text-3xl font-extrabold text-slate-600 group-hover:text-[#FF1E27] transition-colors">
                    {domain.id}
                  </span>
                </div>
                ...
                {/* Title & Subtitle */}
                <div>
                  <h3 className="font-syne text-2xl font-bold text-white uppercase tracking-tight group-hover:text-[#FF1E27] transition-colors">
                    {domain.title}
                  </h3>
                  <div className="font-mono text-xs text-[#FF1E27] mt-1 uppercase tracking-widest">
                    {domain.subtitle}
                  </div>
                </div>
                ...
                {engineerMode && domain.codeSnippet && (
                  <div className="bg-[#0A0A0A] border border-[#FF1E27]/30 p-3 text-[11px] text-[#FF1E27] overflow-x-auto">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {domain.skills.map((skill) => (
                    <span
                      key={skill}
                      className="bg-white/5 border border-white/10 text-slate-300 font-mono text-xs px-2.5 py-1 group-hover:border-[#FF1E27]/30 transition-colors"
                    >
                      {skill}
                    </span>
                  ))}
              ...
              <div className="pt-6 mt-6 border-t border-white/10 flex items-center justify-between font-mono text-xs text-slate-400">
                ...
                <ArrowUpRight size={18} className="group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-[#FF1E27] transition-transform" />
              </div>
            </motion.div>
          ))}
        </div>
