...
export default function Experience({ engineerMode }) {
  ...
  return (
    <section id="experience" className="py-24 px-6 md:px-12 bg-[#050507] relative overflow-hidden border-t border-white/10 font-mono">
        ...
        <div className="space-y-6">
          {experiences.map((exp, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-[#0f0f13] border border-white/15 rounded-sm overflow-hidden transition-all duration-300"
              >
                {/* Header Row */}
                <div
                  onClick={() => setExpanded(isOpen ? null : index)}
                  className="p-6 flex flex-wrap justify-between items-center gap-4 cursor-pointer hover:bg-white/5 transition-colors"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-[#ccff00] font-bold tracking-widest">
                        {exp.year}
                      </span>
                      <span className="text-[10px] text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-sm border border-purple-500/30">
                        {exp.type}
                      </span>
                    </div>

                    <h3 className="font-syne text-xl md:text-2xl font-bold text-white uppercase">
                      {exp.role}
                    </h3>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="hidden md:flex gap-1.5">
                      {exp.tech.slice(0, 4).map(t => (
                        <span key={t} className="bg-white/5 text-slate-400 text-[10px] px-2 py-0.5 rounded-sm border border-white/10">
                          {t}
                        </span>
                      ))}
                    </div>
                    {isOpen ? <ChevronUp size={20} className="text-[#ccff00]" /> : <ChevronDown size={20} className="text-slate-400" />}
                  </div>
                </div>
                ...
                {isOpen && (
                  <motion.div
                    ...
                    className="p-6 pt-0 border-t border-white/10 space-y-4 font-sans text-sm text-slate-300"
                  >
                    <p className="leading-relaxed font-mono text-xs text-slate-200">
                      {exp.summary}
                    </p>

                    <div className="space-y-2 pt-2">
                      <div className="text-xs font-mono text-[#ccff00] uppercase font-bold">KEY DELIVERABLES:</div>
                      {exp.details.map((d, i) => (
